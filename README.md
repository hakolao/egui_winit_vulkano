...
let mut gui = Gui::new(renderer.surface(), renderer.queue(), false, target_color_format);
// Or with subpass. This means that you must create the renderpass yourself. Egui subpass will then draw on your
// image.
let mut gui = Gui::new_with_subpass(renderer.surface(), renderer.queue(), subpass, target_color_format);
```
